...
try:
    message_per_band = open('/home/gm4slv/coaststation/message_per_band.json')
...
try:
    messages_received = open('/home/gm4slv/coaststation/messages_received.json')
...
try:
    excluded = open('/home/gm4slv/coaststation/exclude.json')
...
try:
    freq_allowed = open('/home/gm4slv/coaststation/freq_allowed.json')
...
try:
    allowed_users = open('/home/gm4slv/coaststations/allowed_users.json')
...
def write_file(text):
    global debug
    filename = r'/home/gm4slv/coaststation/coast_station_log.txt'
    if debug == 1:
...
def write_admin(text):
    global debug
    filename = r'/home/gm4slv/coaststation/coast_station_adminlog.txt'
    if debug == 1:
        f = open(filename, 'a+')  # a+ is "append to file, create it if it doesn't exist"
        timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
        log = ";".join((timenow, text))  # make an entry for the log by joining the timestamp with the text passed in
...
def write_txlog(text):
    global debug
    filename = r'/home/gm4slv/coaststation/coast_station_txlog.txt'
    if debug == 1:
        f = open(filename, 'a+')  # a+ is "append to file, create it if it doesn't exist"
        timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
        log = ";".join((timenow, text))  # make an entry for the log by joining the timestamp with the text passed in
...
def write_rxtx(text):
    global debug
    filename = r'/home/gm4slv/coaststation/coast_station_rxtxlog.txt'
    if debug == 1:
        f = open(filename, 'a+')  # a+ is "append to file, create it if it doesn't exist"
        timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
        log = ";".join((timenow, text))  # make an entry for the log by joining the timestamp with the text passed in
    ...
    def dsc(self, data):
        ...
        if self.exclude_flag == 1:
            ...
            if any("~" in s for s in self.dsc_list):
                ...
                with open('/home/gm4slv/coaststation/messages_received.json', 'wb') as outfile:
    ...
    while True: # run forever......
        ...
        if dsc_queue: # there's something to send....
            ...
            if tx_ok == 1: # we can transmit.....
                ...
                # save the list onto disk    
                with open('/home/gm4slv/coaststation/message_per_band.json', 'wb') as outfile:
...
class ThreadedRequestHandler(SocketServer.StreamRequestHandler):
    ...
    def handle(self):
        ...
        self.ipport = str(self.client_address[0])+":"+str(self.client_address[1])
        ...
        while True:
            ...
            if not cmd:
                ...
                sql.load_admin("Admin; Client %s gone..." % self.ipport)
                del online_users[self.ipport]
                #sql.load_admin("Admin; connected clients:")
                #for k,v in online_users.items():
                #    sql.load_admin("Admin; Users online %s (%s)" % (v, k))
            ...
            # a rudimentary ACCESS control list
            try:
                allowed_users = open('/home/gm4slv/coaststation/allowed_users.json')
                allowed_users_list = json.load(allowed_users)
            except:
                allowed_users_list = ["john" ]
                
            #allowed_users_list = ["john", "rich", "dirk", "guest"]
            # the client sends an ID word as the final word in each command. We compare the received 
            # ID against the list of allowed_users
            self.id = cmd.split(";")[-1].lower().strip()  
            #print cmd
            if self.id not in allowed_users_list:

                self.respond="User Invalid --- goodbye"
                self.send_response(self.respond)
                #prompt = str(len(self.respond))+"@"
                #self.wfile.write(prompt+self.respond)
                # Log it....
                write_file("Admin GUI: USER INVALID %s (%s)\n\n" % (self.client_address[0], self.id))
                write_admin("Admin GUI: USER INVALID %s (%s)\n\n" % (self.client_address[0], self.id))
                sql.load_admin("Admin; USER INVALID %s (%s)" % (self.ipport, self.id))
                try:
                    del online_users[self.ipport]
                except:
                    pass
                break
            # 
            # The remote client can request disconnection, which is a tidy way to close the session
            # a simple string is detected
            if cmd.split(";")[1].lower().strip() == "iwantotquit":
                print "Client wants to quit"
                
                # the same procedure for sending length along with data
                self.respond="..... server says goodbye"
                self.send_response(self.respond)
                #prompt = str(len(self.respond))+"@"
                #self.wfile.write(prompt+self.respond)
                ...
                del online_users[self.ipport]
                # Log it....
                write_file("Admin GUI: Logoff from %s (%s)\n\n" % (self.client_address[0], self.id))
                write_admin("Admin GUI: Logoff from %s (%s)\n\n" % (self.client_address[0], self.id))
                sql.load_admin("Admin; Logoff from %s (%s)" % (self.ipport, self.id))
                #sql.load_admin("Goodbye...")
                #sql.load_admin("Admin; Client %s quits" % self.ipport)
                #sql.load_admin("Admin; connected clients:")
                #for k,v in online_users.items():
                #    sql.load_admin("Admin; Users online %s (%s)" % (v, k))
    ...
    def admin(self, command):
        ...
        if self.command[1] == "TESTING":
            self.uptime = get_uptime(start_time)
            # the response procedure should go in a separate function!!
            self.respond = "User %s : Coast Station online (up %s)" % (self.id.title(), self.uptime())
            self.send_response(self.respond)
            ...
            self.send_response(self.respond)
            online_users[self.ipport] = self.client_string
            #sql.load_admin("Hello...")
            
            sql.load_admin("Admin; Client hello: %s %s" % (self.ipport, self.id))
            ...
            #sql.load_admin("Admin; User %s logged in from %s" % (self.id, self.client_address[0]))
        elif self.command[1] == "MOTD":
            with open ("/home/gm4slv/coaststation/motd.txt", "r") as myfile:
        ...
        # The command to inhibit the TX, to prevent any transmissions
            ...
            sql.load_admin("Admin; TX Inhibit applied by %s (%s)" % (self.ipport, self.id))
        ...
        # The command to enable the TX
            ...
            sql.load_admin("Admin; TX Inhibit removed by %s (%s)" % (self.ipport, self.id))
        ...
        # Sent by the client as e.g. "DEL;2187.5"
            ...
            if freq_d in freq_allowed_list:
                ...
                with open('/home/gm4slv/coaststation/freq_allowed.json', 'wb') as outfile:
                ...
                sql.load_admin("Admin; Removed %s from allowed frequencies by %s (%s)" % (freq_d, self.ipport, self.id) )
        ...
        # A command to add a frequency to the allowed_list
            ...
            if freq_a != "" and freq_a not in freq_allowed_list:
                ...
                sql.load_admin("Admin; Added %s to allowed frequencies by %s (%s)" % (freq_a, self.ipport, self.id))
        ...
        # a command to add a receiver to the "EXCLUDED RX" list - i.e. "ADD = EXCLUDE"
            ...
            if rx != "" and rx not in excluded_list:
                ...
                # update the json disk file, for persistence across restarts
                with open('/home/gm4slv/coaststation/exclude.json', 'wb') as outfile:
                ...
                sql.load_admin("Admin; Added %s to excluded RX list by %s (%s)" % (rx, self.ipport, self.id))
        ...
        # a command to remove a RX from the "excluded" list - i.e. "DEL = INCLUDE"       
            ...
            if rx in excluded_list:
                ...
                sql.load_admin("Admin; Removed %s from excluded RX list by %s (%s)" % (rx, self.ipport, self.id))
        ...
        # a command to reset the message counts to ZERO
            ...
            sql.load_admin("Admin; Message Count reset by %s (%s)" % (self.ipport, self.id))
        ...
        # not running.
            ...
            sql.load_admin("Admin; DATABASE PURGED FROM %s (%s)" % (self.ipport, self.id))
