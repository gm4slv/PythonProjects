...
# CW ID for use on Amateur Radio Bands
# with Icom IC-7200 control


# Imports
from Tkinter import *
from dsc_functions import *
from dummy_radio_functions_m710 import *
...
class Application(Frame):
    ...
    def create_widgets(self):
        # get the current Radio dial freq & convert to DSC Centre Freq
        freq = float(get_freq()) + 1.7
        # force Radio into USB / Digital 
        set_mode("usb")
        set_digi()
        
        # a Frame to hold the radio frequency setting widgets
        radio_frame = Frame(root, relief = GROOVE, borderwidth = 2, pady = 5)
        radio_frame.grid(row = 20, column = 0, padx = 5, pady = 5, sticky = W+E)
        ...
        self.go_b = Button(self, text = "Send", command = self.dscqueue_on)
        self.go_b.grid(row = 10, column = 0, sticky = W+E)
        
        
        #self.tune_b = Button(self, text = "Tune", command = self.tune)
        ...
        self.dsc_l = Label(dsc_call_f, textvariable = self.dsc_label, fg = 'blue', height = 2, wraplength = 350, anchor = W)
        self.dsc_l.grid(row = 0, column = 0)
        
        #### Radio Settings
        # Frequency set - Labels to show things
        self.freq_label = Label(radio_frame, text = "Current Freq: ",fg = 'blue').grid(row = 0, column = 0, sticky = E )
        
        self.freq_label = StringVar()
        self.freq_l = Label(radio_frame, textvariable = self.freq_label, fg = 'blue').grid(row = 0, column = 1, columnspan = 2, sticky = W)
        self.freq_label.set(freq)
        
        # Button to set new freq (if neccesary) and then fire off 3-second tuning carrier
        self.tune_b = Button(radio_frame, text = "Tune >", command = self.tunequeue_on)
        self.tune_b.grid(row = 1, column = 0, sticky = W, pady = 5, padx = 5)
        
        # Choice of frequency in a Spinbox  - can be edited to directly set a new frequency
        self.freq_sp = Spinbox(radio_frame, width = 10, wrap = True, values = ("5368.0", "2177.0", "2187.5", "4207.5", "6312.0", "8414.5", "12577.0", "16804.5"))
        self.freq_sp.grid(row = 1, column = 1, sticky = W)
        
        # TX Power Control
        self.pwr_l = Label(radio_frame, width = 10, text = "Power").grid(row = 2, column = 0)
        self.pwr_sp = Spinbox(radio_frame, width = 4, wrap = True, values = (0.7, 1.0, 0.2, 0.5))
        self.pwr_sp.grid(row = 2, column = 1)
    ...
    def tune(self):
        while True:
            ...
            if t_on == 1:
                ...
                set_mode("usb")
                set_digi()
                pwr = float(self.pwr_sp.get())
                try:
                    freq = float(self.freq_sp.get()) - 1.7
            
                    set_freq(freq)
                    #print freq
        
                    txfreq = float(get_freq()) + 1.7
                    self.freq_label.set(txfreq)
                except:
                    pass
