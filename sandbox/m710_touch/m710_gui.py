...
import tkFont
import subprocess
import dummy_radio_functions_m710 as radio
import json
import time
...
class m710(object):
    def __init__(self, master):
        ...
        self.qsy1k_r = Radiobutton(delta_frame,  indicatoron=0,activebackground=button_colour,bg=button_colour,  selectcolor=select_colour, width=6,text="5kHz", variable=self.delta, value="5")
        self.qsy1k_r.grid(row = 1, column = 3, padx=3,pady=2, sticky="NEWS")
        self.qsy100h_r = Radiobutton(delta_frame, indicatoron=0,activebackground=button_colour,bg=button_colour, selectcolor=select_colour, width=6, text="1kHz", variable=self.delta, value="1")
        self.qsy100h_r.grid(row = 1, column = 4,padx=3,pady=2, sticky="NEWS")
        self.qsy10h_r = Radiobutton(delta_frame, indicatoron=0,activebackground=button_colour,bg=button_colour,  selectcolor=select_colour, width=6, text="100Hz", variable=self.delta, value="0.1")
        ...
        mode_frame.grid(row = 6, column = 0, columnspan = 5, padx=2, pady=2, sticky="W")
        ...
        self.usb_r.grid(row = 1, column = 0,padx=3,pady=3, sticky="NESW")
        ...
        self.lsb_r.grid(row = 1, column = 1,padx=3,pady=3, sticky="NESW")
        ...
        self.cw_r.grid(row = 1, column = 2,padx=3,pady=3, sticky="NESW")
        ...
        self.am_r.grid(row = 1, column = 3,padx=3,pady=3, sticky="NESW")
        
        
        button_frame = Frame(control_frame, bg=panel_colour, borderwidth=2, relief=GROOVE)
        button_frame.grid(row = 8, column = 0, columnspan = 5, pady=5, sticky="W")
        
        self.sp_cb = Checkbutton(button_frame,indicatoron=0,activebackground=button_colour, bg=button_colour, selectcolor=select_colour, height=2, width=3, text = "Spk", variable=self.sp, command = lambda:self.speaker())
        self.sp_cb.grid(row=0, column=0,padx=3,pady=2,sticky="NSEW")
        
       
        self.dim_cb = Checkbutton(button_frame, indicatoron=0,activebackground=button_colour,bg=button_colour,  selectcolor=select_colour, height=2, width=3,text = "Dim", variable=self.dim, command = lambda:self.dimc())
        ...
        self.txp3_r = Radiobutton(button_frame,activebackground=button_colour,bg=button_colour,  text = "Hi",selectcolor=select_colour, height=2,width=3, indicatoron=0,  variable=self.txp, value = "3", command = lambda: self.txpc("3"))
        self.txp3_r.grid(row=2, column=2,padx=2, pady=2, sticky="NSEW")
        self.txp2_r = Radiobutton(button_frame,activebackground=button_colour,bg=button_colour,  text = "Mid",selectcolor=select_colour, height=2,width=3, indicatoron=0,  variable=self.txp, value = "2", command = lambda: self.txpc("2"))
        self.txp2_r.grid(row=2, column=1,padx=2, pady=2, sticky="NSEW")
        self.txp1_r = Radiobutton(button_frame, activebackground=button_colour,bg=button_colour, text = "Low",selectcolor=select_colour, height=2, width=3, indicatoron=0,  variable=self.txp, value = "1", command = lambda: self.txpc("1"))
        self.txp1_r.grid(row=2, column=0, padx=3, pady=2, sticky="NSWE")
        ...
        self.remoteon_b = Radiobutton(button_frame,activebackground=button_colour, bg=button_colour, text="On", indicatoron=0, height=2, width=3, selectcolor=select_colour, variable=self.remote_flag, value=1, command = lambda: self.remote_toggle())
        self.remoteon_b.grid(row = 3, column = 0,padx=3,pady=2, sticky="NSEW")
        
        self.remoteoff_b = Radiobutton(button_frame,activebackground=button_colour, bg=button_colour, text="Off", selectcolor=select_colour, height=2, width=3,indicatoron=0, variable=self.remote_flag, value=0, command = lambda: self.remote_toggle())
        self.remoteoff_b.grid(row = 3, column = 1,padx=2,pady=2, sticky="NSEW")
        
        self.sd_b = Button(button_frame,activebackground=button_colour, bg=button_colour,width=3, height=2, text="X", command= lambda: self.shutdown_pi())
        self.sd_b.grid(row=3, column=2, padx=2, pady=2, sticky="NSEW")
       
        ###################
        #
        # Memory Frame

        mem_frame = Frame(control_frame, bg=panel_colour, borderwidth=2, relief=GROOVE)
        mem_frame.grid(row=8, column=2, rowspan=4, columnspan=4, sticky="NSEW") 

        self.mem1_b = Button(mem_frame, text="1", height=2, width=3,  command=lambda: self.memtune("1"))
        self.mem1_b.grid(row=0, column=0, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="2",  height=2, width=3,  command=lambda: self.memtune("2"))
        self.mem1_b.grid(row=0, column=1, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="3", height=2, width=3,  command=lambda: self.memtune("3"))
        self.mem1_b.grid(row=0, column=2, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="4", height=2, width=3,  command=lambda: self.memtune("4"))
        self.mem1_b.grid(row=0, column=3, padx=2, pady=2)
        
        self.mem1_b = Button(mem_frame, text="5",  height=2, width=3,  command=lambda: self.memtune("5"))
        self.mem1_b.grid(row=1, column=0, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=1, column=1, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=1, column=2, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=1, column=3, padx=2, pady=2)
        
        self.mem1_b = Button(mem_frame, text="5",  height=2, width=3,  command=lambda: self.memtune("5"))
        self.mem1_b.grid(row=2, column=0, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=2, column=1, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=2, column=2, padx=2, pady=2)
        self.mem1_b = Button(mem_frame, text="6", height=2, width=3,  command=lambda: self.memtune("6"))
        self.mem1_b.grid(row=2, column=3, padx=2, pady=2)
        ...
        self.sql_cb = Checkbutton(button_frame,indicatoron=0,activebackground=button_colour, bg=button_colour, selectcolor=select_colour, height=2, width=4, text = "SQL", variable=self.sql, command = lambda:self.sqlc())
        self.sql_cb.grid(row=0, column=1,padx=3,pady=2,sticky=N+S+W+E)
        self.nb_cb = Checkbutton(button_frame,indicatoron=0,activebackground=button_colour, bg=button_colour, selectcolor=select_colour, height=2, width=4, text = "NB", variable=self.nb, command = lambda:self.nbc())
        self.nb_cb.grid(row=0, column=2,padx=3,pady=2,sticky=N+S+W+E)
        ...
        self.tune_b.grid(row = 4, column = 0,padx=2,pady=2, sticky="NSEW")    
