...
def start():
    ...
    if len(data.split()) > 1:
        ...
        cur_freq = connect("getfreq" + " " + radio_num)
        print "%s replied : current %s kHz" % (rname, cur_freq)
        new_freq = float(cur_freq) + 0.500
        print "new freq ", new_freq
        freq = connect("setfreq" + " " + str(new_freq) + " " + radio_num)
