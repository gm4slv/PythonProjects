...
def resolve_mmsi(mmsi):
    ...
    if ship != None:
        ...
        print "+++++ Resolve2 says name2 %s, call2 %s, vt2 %s " % (name2, call2, vt2)
    else:
        print "+++++ Not found at resolve2"
    ...
    # not found - we need to try the resolution methods in turn
        ...
        if name_call != None:
            ...
            #write_resolve_log("")
            #write_resolve_log("Trying ITU for %s" % mmsi)
	    #pysql.load_logger("RESOLVER", "Trying ITU for %s" % mmsi)
	    pysql.load_logger("RESOLVER", "ITU MARS NOT USABLE : %s" % mmsi)
            name_call = resolve_itu(mmsi)
            if name_call != None:
                pysql.load_logger("RESOLVER", "Found at ITU {%s : %s}" % (mmsi, name_call))
            elif name_call == None:
                #write_resolve_log("")
        ...
        # update the dictionary with the latest resolved name
        #write_resolve_log("Adding new record to ship databases {%s : %s}" % (mmsi, name_call))
        
        pysql.load_logger("RESOLVER", "Adding new record to ship database : %s { %s }" % (mmsi, name_call))
        
        #try:
        #    pysql.add_shipname(mmsi, name_call)
        #    
        #except:
        #    pass
...
def resolve_itu(mmsi): 
    ...
    url = 'http://www.itu.int/online/mms/mars/ship_search.sh'
    values = {"lang": "en", "lgg": 7, "p": 1, "sh_name": "", "sh_callsign" : "", "sh_mmsi" : mmsi, "cgaid" : 0, "sh_epirb_id" : "", "sh_epirb_hex" : "", "sh_imo_nbr" : ""}

    data = urllib.urlencode(values)
    req = urllib2.Request(url, data)
    #write_resolve_log("")
    #write_resolve_log("Looking up %s at ITU" % mmsi)
    #pysql.load_logger("RESOLVER", "Looking up %s at ITU" % mmsi)
    response = urllib2.urlopen(req)
    the_page = response.read()

   

    no_result = re.findall(r'No records found for given criteria', str(the_page))
    if no_result:
        #write_resolve_log("Not found at ITU: %s" % mmsi)
        pysql.load_logger("RESOLVER", "Not found at ITU: %s" % mmsi)
        
        return None
     
    #paragraphs = re.findall(r'<A HREF(.*?)</A>',str(the_page))
    name_line = re.findall(r'ship_detail.*', str(the_page))

    call_line = re.findall('(.*?)\</td', str(the_page))
    for eachP in name_line:
        #print(eachP)
    
        #print(eachP)
        name = re.findall('\>(.*?)\<', eachP)
        #print name
        #name = name[0].title().replace(',', '').strip()
        name = name[0].upper().replace(',', '').strip()
        print name

    call_mmsi = []
    
    for eachP in call_line:
        #print (eachP)
    
        if not '<' in eachP and not '\t' in eachP:
            call_mmsi.append(eachP)
    print call_mmsi
    try:
        call = call_mmsi[3]
        #mmsi = call_mmsi[4]
    ...
    name_call = name + ", " + call + ", N/A"
    
    print name_call

    #write_resolve_log("New MMSI found at ITU: %s : %s" % (mmsi, name_call))
    #pysql.load_logger("RESOLVER", "New MMSI found at ITU: %s : %s" % (mmsi, name_call))
