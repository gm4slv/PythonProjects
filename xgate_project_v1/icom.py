"""
GM4SLV Icom Radio Class for CiV control
...
######################################
#
# A class for Icom CiV radios for remote 
# control. 
# 
# useage: instantiate with 
# r1 = Icom(n1,a1,cal1)
#
# n1, a1 and cal1 are defined in the supporting "conf.py"
#
...
class Icom(object):
    ...
    def bandwidth_wide(self):
        sendStr = preamble + preamble + self.radio_address + controller + filter_wide_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def bandwidth_normal(self):
        sendStr = preamble + preamble + self.radio_address + controller + filter_normal_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def bandwidth_narrow(self):
        sendStr = preamble + preamble + self.radio_address + controller + filter_narrow_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def mod_a(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_mod_a_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def mod_am(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_mod_am_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def comp_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + comp_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def backlight_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + backlight_off_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def vfo_mode(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_vfo_mode + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def backlight_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + backlight_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dial_lock_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + dial_lock_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dial_lock_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + dial_lock_off_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dsp_nr_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + dspnr_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dsp_anf_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + dspanf_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dsp_nr_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + dspnr_off_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def dsp_anf_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + dspanf_off_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def nb_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + nb_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def nb_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + nb_off_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def tune(self):
        ...
        self.set_pwr(curpwr)
        #print "Power now...", self.get_pwr()
        #print "Tuned : (ref pwr : %s)" % swr
        return "ATU Tune cycle complete"
