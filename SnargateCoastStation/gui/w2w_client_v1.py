...
class Application(Frame):
    def __init__(self, master):
        ...
        # Start polling the Status information - fast update
        self.poll()
        
        # Start polling the log-file informatin - slow update
        self.poll_win()
    ...
    # Use a tkMessageBox widget to display "About...."
    def about(self):
    ...
    def create_widgets(self):
        ...
        logmenu.add_command(label="All RX", command=self.rx_win)
        logmenu.add_command(label="All TX", command=self.tx_win)
        logmenu.add_command(label="All MMSI", command=self.mmsi_win)
        logmenu.add_command(label="All Syslog", command=self.allsys_win)
        
        
        menubar.add_cascade(label="Logs", menu=logmenu, underline=0)
        ...
        # frame called as Toplevel via the Menu
        ...
        # hide (don't destroy) the Toplevel window
        ...
        #######################
        #
        # widget in the Command replies from the Server
        #
        # A Label showing the last reply
    ...
    def make_con(self):
        ...
        if self.con_ok:
            ...
            n1.make_con()
            self.con_count += 1 # increment the counter each time a connection is made - to keep track of re-connects
            self.connected=1
            self.lost_comms=0
            self.send_command("hello")
            self.id_e.configure(state=DISABLED) # disable the User Name entry widget once we've connected (re-enabled on disconnect)
        else:
            self.close()
    ...
    def mmsi_win(self):
        ...
        if self.mmsi_frame_exists == 0:
            ...
            self.mmsimenu = Menu(self.mmsi_frame)
            # save the text to a file....
            self.mmsimenu.add_command(label="Save...", command= lambda: self.file_save(self.mmsi_term.get(1.0, END)))
            # force a refresh of the data, the poll_win()  function may not run often enough.
            self.mmsimenu.add_command(label="Refresh...", command = lambda: self.get_mmsi_log())
            self.mmsi_frame.config(menu=self.mmsimenu)
        
            self.mmsi_term = ScrolledText(self.mmsi_frame, fg='blue',bg='azure2', width = 50, height = 25, wrap = WORD)
    ...
    def sys_win(self):
        ...
        if self.sys_frame_exists == 0:
            ...
            self.sys_frame.grid() 
            self.sysmenu = Menu(self.sys_frame)
            
            self.sysmenu.add_command(label="Save...", command= lambda: self.file_save(self.sys_term.get(1.0, END)))
            self.sysmenu.add_command(label="Refresh...", command = lambda: self.get_sys_log())
            self.sys_frame.config(menu=self.sysmenu)
        
            self.sys_term = ScrolledText(self.sys_frame, fg='blue', bg='azure2', width = 120, height = 25, wrap = WORD)
    ...
    def allsys_win(self):
        ...
        if self.allsys_frame_exists == 0:
            self.allsys_frame = Toplevel(self, borderwidth = 2, relief = GROOVE, bg = 'black')
            self.allsys_frame.title("System Log")
            ...
            self.allsysmenu.add_command(label="Refresh...", command = lambda: self.get_allsys_log())
            self.allsys_frame.config(menu=self.allsysmenu)
        
            self.allsys_term = ScrolledText(self.allsys_frame, fg='blue', bg='azure2', width = 120, height = 25, wrap = WORD)
    ...
    def rx_win(self):
        ...
        if self.rx_frame_exists == 0:
            ...
            self.rxmenu = Menu(self.rx_frame)
            
            self.rxmenu.add_command(label="Save...", command= lambda: self.file_save(self.rx_term.get(1.0, END)))
            self.rxmenu.add_command(label="Refresh...", command = lambda: self.get_rx_log())
            
            self.rx_frame.config(menu=self.rxmenu)
            
            self.rx_term = ScrolledText(self.rx_frame, fg='blue',bg='azure2', width = 100, height = 25, wrap = WORD, font=('courier new',10,''))
    ...
    def rxtx_win(self):
        ...
        if self.rxtx_frame_exists == 0:
            ...
            self.rxtxmenu = Menu(self.rxtx_frame)
            
            self.rxtxmenu.add_command(label="Save...", command= lambda: self.file_save(self.rxtx_term.get(1.0, END)))
            self.rxtxmenu.add_command(label="Refresh...", command = lambda: self.get_rxtx_log())
            self.rxtx_frame.config(menu=self.rxtxmenu)
            
            self.rxtx_term = ScrolledText(self.rxtx_frame, fg='blue', bg='azure2', width = 120, height = 25, wrap = WORD, font=('courier new',10,''))
    ...
    def tx_win(self):
        ...
        if self.tx_frame_exists == 0:
            ...
            self.tx_frame.grid() 
            self.txmenu = Menu(self.tx_frame)
            
            self.txmenu.add_command(label="Save...", command= lambda: self.file_save(self.tx_term.get(1.0, END)))
            self.txmenu.add_command(label="Refresh...", command = lambda: self.get_tx_log())
            self.tx_frame.config(menu=self.txmenu)
        
            self.tx_term = ScrolledText(self.tx_frame, fg='red', bg='azure2', width = 100, height = 25, wrap = WORD, font=('courier new',10,''))
    ...
    def close(self):
        ...
        self.id_e.configure(state=NORMAL) # restore the User Name entry widget to normal
    ...
    # pop up a Quit/Verify box to double check...
    def shutdown(self):
    ...
    def online(self):
        ...
        if re.search("online", online):     # check for the presence of "online" in the reply string
            ...
            self.snargate_reply1.set(online+" : Connect count: "+ self.count)            # set the status label text to the returned text string from the server (plus a count of haw many times we've connected)
    ...
    def get_last_rx_message(self):
        ...
        if self.lost_comms == 1 and self.connected == 1:
            ...
            self.l_reply4.configure(fg = 'blue')        
            self.snargate_reply4.set("Last RX: "+last_message)
    ...
    def get_last_tx_message(self):
        ...
        if self.lost_comms == 1 and self.connected == 1:
            ...
            self.l_reply5.configure(fg = 'red')
            self.snargate_reply5.set("Last TX: "+last_tx)
        '''    
        if self.lost_comms == 1 and self.connected == 1:
            self.l_reply5.configure(fg = 'red')
            self.snargate_reply5.set("Lost Comms")
    ...
    # this function retrieves the currently allowed frequencies from the server and
    ...
    def get_allsys_log(self):
        allsys_log = self.poll_cmd("GETALLADMINSQL")
    ...
    # function called regularly by poll_win() to retrieve the system log data and update the sys_term TopLevel window
    ...
    # function called regularly by poll_win() to retrieve the RX/TX log data and update the rxtx_term TopLevel window       
    def get_rxtx_log(self):
        ...
        #    print "exception"
            pass
    # function called regularly by poll_win() to retrieve the RX log data and update the rx_term TopLevel window       
    def get_rx_log(self):
    ...
    # function called regularly by poll_win() to retrieve the TX log data and update the tx_term TopLevel window
    def get_tx_log(self):
    ...
    # a delay of 2 seconds, during which time application control returns to the user
    ...
    def poll(self):
        ...
        self.master.after(2000, self.poll)
        #time.sleep(1)
    ...
    def poll_win(self):
        ...
        #self.get_tx_log()
        self.master.after(30000, self.poll_win)  # 30 seconds between log display updates
...
# Called when the main window's "Close" cross-button is clicked    
def askquit():
    if askyesno('Verify', 'Really quit?', default=NO):
