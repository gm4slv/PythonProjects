...
class Application(Frame):
    ...
    def get_allsys_log(self):
        ...
        try: # in a try/except because the term may not exist if the user hasn't pressed the button, but the function is still called by poll() anyway
            ...
            self.allsys_term.insert(END, self.allsys_log+"\n")
    ...
    def get_allrxtx_log(self):
        ...
        try: # in a try/except because the term may not exist if the user hasn't pressed the button, but the function is still called by poll() anyway
            self.allsys_term.config(state='normal')
            self.allsys_term.delete(0.0, END)
            self.allsys_term.insert(END, self.allrxtx_log+"\n")
            self.allsys_term.yview(END)
            self.allsys_term.config(state='disabled')
        except:
            pass
    ...
    def update_mmsi(self):
        ...
        with open('mmsi_send.json', 'wb') as outfile:
    ...
    def create_widgets(self):
        ...
        self.sys_note.add(self.f1, text='Controls')
        self.sys_note.add(self.f2, text='RX/TX Log')
        self.sys_note.add(self.f3, text='Syslog')
        
        
        self.sys_note.grid(row=1, column=0)
        
        self.button_3_frame = Frame(self.f1, bg="#b0c4de")
        self.button_3_frame.grid(row = 0, column = 0, columnspan=14, rowspan = 10,sticky=W+E)
        
        self.cb_frame = Frame(self.f1, bg='#b0c4de')      
        self.cb_frame.grid(row=15, column=0, columnspan = 10, sticky=N+S+E+W)
        
        ######### THE TEXT AREAS ##############
        self.sys_term = ScrolledText(self.f3, takefocus=0, pady=0, borderwidth=0, fg='black', bg='#b0c4de', width = 120, height = 18, wrap = WORD)
        self.sys_term.grid(row = 1, column = 0, columnspan = 14,  sticky=W+E+N+S)
         
        self.rxtx_term = ScrolledText(self.f2, takefocus=0, pady=0, borderwidth=0,  fg='blue', bg='#b0c4de', width = 120, height = 18, wrap = WORD)
        self.rxtx_term.grid(row = 1, column = 0, columnspan = 14,  sticky=W+E+N+S)
        
        #self.mmsi_term = ScrolledText(self.f3, takefocus=0, fg='darkgreen',  pady=0, borderwidth=0, bg='#b0c4de',  width = 120, height = 18, wrap = WORD)
        ...
        self.dsc_l.grid(row = 0, column = 0,pady=5, columnspan=3)
        ...
        self.call1_b.grid(row = 1, column = 5,  sticky = W+E)
        
        # Button to send the command to generate a DSC Test Call addressed to ourselves, on 12MHz
        self.call2_b = Button(self.button_3_frame, padx=3, text = "12MHz Self Call", fg='white', bg='#6699cc', command = lambda: poll_cmd("CALL;002320204;12577.0"))
        self.call2_b.grid(row = 1, column = 4, padx=3,  sticky = W+E)
        ...
        self.call3_b.grid(row = 1, column = 3,  padx=3,  sticky = W+E)
        ...
        self.call5_b.grid(row = 1, column = 2,  padx=3, sticky = W+E)
        ...
        self.call4_b.grid(row = 1, column = 1, padx=3, sticky = W+E)
        ...
        self.call6_b.grid(row = 1, column = 0,  padx=3, sticky = W+E)
        '''
        
        
        
        #self.blank = Label(self.button_3_frame,  bg='#b0c4de')
        #self.blank.grid(row = 3, column = 0, columnspan = 3, padx=3, sticky=W+E)
        
        self.dsc_l = Label(self.button_3_frame, text = "DSC Test Call" , bg='#b0c4de', font=('',9,'bold'))
        self.dsc_l.grid(row = 4, column = 0, columnspan=3)
        
        self.dscto_l = Label(self.button_3_frame, text = "To MMSI" , bg='#b0c4de')
        self.dscto_l.grid(row = 5, column = 0,padx=3, sticky = W+E)
        self.dscf_l = Label(self.button_3_frame, text = "Frequency" , bg='#b0c4de')
        self.dscf_l.grid(row = 5, column = 1,padx=3, sticky = W+E)
        ...
        self.to_mmsi_e = Spinbox(self.button_3_frame,width=10,justify=CENTER, wrap = True, values = ("002320204","002320001", "002320004", "002320024", "002191000", "002711000", "002241022", "003669991"), font=('',9,'bold'))
        self.to_mmsi_e.grid(row = 6, column = 0,pady=3, padx=3, sticky = N+S+W+E)
        '''
        self.to_mmsi_val = StringVar()
        self.to_mmsi_e = ttk.Combobox(self.button_3_frame, width=10, font=('',9,'bold'), textvariable = self.to_mmsi_val)
        self.to_mmsi_e.grid(row = 6 , column = 0, padx = 5,)
        #self.to_mmsi_e['values'] = ("002320204","002320001", "002320004", "002320024")
        #self.to_mmsi_e.current(0)
        #self.to_mmsi_e.bind('<Return>', self._update_values)
        #self.to_mmsi_e.bind('<Tab>', self._update_values)
        '''
        ...
        self.freq_sp = Spinbox(self.button_3_frame,width=10, justify = CENTER, wrap = True, values = ("2187.5", "4207.5", "6312.0", "8414.5", "12577.0", "16804.5"),font=('',9,'bold'))
        self.freq_sp.grid(row = 6, column = 1,pady=3, padx=3,sticky = N+S+W+E)
        '''
        self.send_freq = StringVar()
        self.freq_sp = ttk.Combobox(self.button_3_frame, font=('',9,'bold'), width=10, textvariable=self.send_freq)
        self.freq_sp.grid(row=6, column = 1)
        self.freq_sp['values']=("2187.5", "4207.5", "6312.0", "8414.5", "12577.0", "16804.5")
        ...
        self.go_b.grid(row = 6, column = 2,pady=3,padx=3, sticky = W)
        ...
        self.reset_b.grid(row = 8, column = 0, columnspan=2,pady=10, padx=3, sticky = W)
        
        #self.blank = Label(self.button_3_frame,  bg='#b0c4de')
        #self.blank.grid(row = 9, column = 0, columnspan = 3, sticky=W+E)
        ...
        self.exclude_b.grid(row = 9, column = 1, pady=2, padx=3, sticky = W+E)
        
        #
        # the entry box, which is read by pressing either "exclude_b" or "include_b"
        self.exclude_e = Entry(self.button_3_frame,width=8)
        self.exclude_e.grid(row = 9, column = 0,pady=2, padx=3, sticky = W+E)
        ...
        self.include_b.grid(row = 10, column = 1, pady=2, padx=3, sticky = W+E)
        
        self.include_e = Entry(self.button_3_frame,width=8)
        self.include_e.grid(row = 10, column = 0,pady=2, padx=3, sticky = W+E)
        self.include_e.bind( '<Return>', (lambda event: self.include_b.invoke()))
        
        self.blank = Label(self.button_3_frame,  bg='#b0c4de')
        self.blank.grid(row = 11, column = 0, columnspan = 3, padx=3, sticky=W+E)
    ...
    def log_poll(self):
        if self.log_queue.qsize():
            ...
            #self.mmsi_term.insert(END, self.mmsi_log+"\n")   # insert the new list
