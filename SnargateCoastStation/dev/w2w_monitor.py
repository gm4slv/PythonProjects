...
def write_file(text):
	...
	<meta http-equiv=\"refresh\" content=\"30\"><meta http-equiv=\"Content-Type\" content=\"text/html;charset=ISO-8859-1\">\
	</head><body bgcolor=\"b0c4de\"><basefont face=\"Arial\">"

    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
    f.write(header+"\n")
    f.write(text+"<center><br><br>Last Update : "+timenow)
    f.write("<br><a href=\"./snargate_syslog.txt\">Syslog</a><br>")
    f.write("<br><a href=\"./snargate_rxtx.txt\">RX/TX Log</a><br>")
    f.write("<br><a href=\"./snargate_txpwr.txt\">TX Power/SWR log</a></body></html>")
    #print timenow + "> " +text
    f.close()
...
def syslog_poll():
    syslog = poll_cmd("GETALLADMINSQL")
    return syslog

def rxtx_poll():
    rxtx = poll_cmd("GETALLRXTXSQL")
    return rxtx
...
def status_handle(status):
    ...
    try:
        ...
        last_tx = "<tr><td><font color=red>Last Transmitted Call</td><td><font color=red> <b>"+last_tx+"</font></b></td></tr>"
        ...
        text = heading+"<center><table cellpadding=5 rules=all>"+online+tx_enable+last_rx+last_tx+message_count+perband+freqs_allowed+"</table></center>"
        write_file(text)
    except:
    	pass
    return
