...
class Application(Frame):
    def __init__(self, master):
        ...
        servermenu.add_command(label="Update MOTD", command=lambda: self.update_motd())
    ...
    def update_motd(self):
        ...
        self.update_motd_allowed = 1
        if self.update_motd_allowed:
            if askyesno('Confirm', 'Really Update MOTD?', default='no'):
                reason = askstring('New MOTD Info', 'Please enter your additions to the MOTD')
                if reason != "":
                    server_reply = poll_cmd("UPDATEMOTD;"+reason)
                    showinfo('Reply', 'MOTD Updates\nServer reply : %s' % server_reply)
                else:
                    showinfo('Reason', 'No MOTD given, update cancelled')
            else:
                showinfo('No', 'Update cancelled')
    ...
    def allsyswin(self):
        ...
        if self.allsys_frame_exists == 0:
            ...
            self.allsysmenu.add_command(label="RX/TX log...", command = lambda: self.get_allrxtx_log())
            self.allsysmenu.add_command(label="RX log...", command = lambda: self.get_allrx_log())
            self.allsysmenu.add_command(label="TX log...", command = lambda: self.get_alltx_log())
            self.allsysmenu.add_command(label="Check log...", command = lambda: self.get_allcheck_log())
            self.allsysmenu.add_command(label="MMSI log...", command = lambda: self.get_mmsi_log())
            self.allsysmenu.add_command(label="Pwr/SWR log...", command = lambda: self.get_txpwr_log())
            self.allsysmenu.add_command(label="Syslog...", command = lambda: self.get_allsys_log())
            self.allsysmenu.add_command(label="Close", command = lambda: self.allsys_frame.destroy())
    ...
    def get_txpwr_log(self):
        self.allcheck_log = poll_cmd("GETALLTXPWRSQL")
        self.allsys_term.config(fg='gray30')
        self.allsys_term.config(state='normal')
        self.allsys_term.delete(0.0, END)
        self.allsys_term.insert(END, self.allcheck_log+"\n")
    ...
    def create_widgets(self):
        ...
        self.l_reply6.grid(row=8, column=0,  sticky=E+W)
        ...
        self.l_reply7.grid(row=9, column=0,  sticky=E+W)
        
        self.snargate_reply9 = StringVar()
        self.l_reply9 = Label(self.status_frame, anchor=SW, justify=LEFT, bg='#b0c4de', textvariable=self.snargate_reply9, font=('sans',9,'bold'))
        self.l_reply9.grid(row=7, column=0, sticky=E+W)
        
        self.blank = Label(self.button_1_frame,  bg='#b0c4de')
        self.blank.grid(row = 0, column = 0, columnspan = 3, padx=3, sticky=W+E)
        
        self.dsc_l = Label(self.button_1_frame, text = "DSC Test Call" , bg='#b0c4de', font=('',9,'bold'))
        ...
        self.go_b = Button(self.button_1_frame, text = "Send", fg='white',  bg='#6699cc',command = lambda: poll_cmd("CALL;"+self.to_mmsi_e.get()+";"+self.freq_sp.get()))
        self.go_b.grid(row = 3, column = 2,pady=3,padx=3, sticky = W)
    ...
    def status_poll(self):
        ...
        root.title("Wire2waves DSC Coast Station Client " + version + "         "+self.datenow+"  "+self.timenow+" UTC ")
        if self.status_queue.qsize():
            ...
            self.lastck = self.status_list[8]
            self.l_reply9.configure(fg = '#666666')
            self.snargate_reply9.set("Last CK: "+self.lastck)
