...
from radio_functions_ic7200 import *
#
############################
...
def shutdown():
    ptt_off()
    #time.sleep(1)
    os.remove('/home/gm4slv/start.txt')
    os._exit(0)
...
# Append text to the MOTD file
def update_motd(text):
    filename = r'motd_tail.txt'
    f = open(filename, 'a+')  # a+ is "append to file, create it if it doesn't exist"
    timenow = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
    log = " : ".join((timenow, text))  # make an entry for the log by joining the timestamp with the text passed in
    f.write(log)
            ...
            if any("~" in s for s in self.dsc_list):
                ...
                if self.from_mmsi == self.our_mmsi: # it's our own transmission reported back
                    if self.to_mmsi[0:2] != "00":
                        ...
                        self.result = resolve_sql.check_mmsi(self.to_mmsi)
                        #self.result = resolve_itu(self.to_mmsi)
                        print "resolver says ", self.result
                        try:
                            
                            self.name = self.result[0]
                            self.call = self.result[1]
                            #self.name = self.result[2]
                            #self.call = self.result[3]
                            print "CHECK LOG!!Name ", self.name
                            sql.load_resolve(self.to_mmsi, self.name, self.call)
                ...
                if self.to_mmsi == self.our_mmsi:
                    ...
                    if self.from_mmsi[0:2] != "00": # not a coast-station 
                        try:
                            ...
                            ##### 
                            #####  use YaDDNEt's own resolve database....
                            self.ship = resolve_sql.check_mmsi(self.from_mmsi)
                            #self.ship = resolve_itu(self.from_mmsi)
                            ...
                            print "Name : %s, MMSI : %s, Call : %s , Vessel Type : %s" % (self.name, self.from_mmsi, self.call, self.vtype)
                            ...
                            write_file("MMSI : %s, Name : %s, Call : %s, Vessel Type : %s\n\n" % (self.from_mmsi, self.name, self.call, self.vtype))
                            ...
                            write_rxtx("MMSI : %s, Name : %s, Call : %s, Vessel Type :  %s\n\n" % (self.from_mmsi, self.name, self.call, self.vtype))
                            ...
                            # log it
                            write_file("Ship data not found at YaDDNet\n")
                            #write_file("Ship data not found at www.aprs.fi\n")
                            write_rxtx("Ship data not found at YaDDNet\n")
                    ...
                    # 16/5/16 : CAT = UNK has been seen in a message to Snargate. (original message had 102 as CAT symbol)
                    # two choices : allow only CAT == "SAF" or catch UNKs with  CAT != "UNK"
                    # to allow "RTN" and disallow "UNK" will try CAT != "UNK"
                    if self.cat != "SAF":
                        print "Incorrect Category %s" % self.cat
                        if self.cat == "UNK":
                            print "Illegal category %s" %self.cat
                            sql.load_admin("Illegal Category %s from %s (%s) call dropped" % (self.cat, self.from_mmsi, self.name))
                        else:
                            sql.load_admin("Incorrect Category %s from %s (%s) - call acknowledge" % (self.cat, self.from_mmsi, self.name))

                    if self.to_mmsi == self.our_mmsi and self.fmt == "SEL" and self.cat != "UNK" and self.tc1 == "TEST" and self.eos == "REQ":
                        print "calling reply()"
    ...
    def reply(self, rx_freq, fmt, to_mmsi, cat, from_mmsi, tc1, tc2, data, eos):
        ...
        if new_dsc_call in dsc_queue:
            ...
            if to_mmsi[0:2] != "00":
                ...
                self.result = resolve_sql.check_mmsi(to_mmsi)
                print "resolver says ", self.result
                self.name = self.result[0]
                self.call = self.result[1]
                #self.name = self.result[2]
            ...
            if tx_ok == 1: # we can transmit
                ...
                sql.load_rxtx(self.sql_time, rx_freq, "TX", to_mmsi, self.name, self.call, self.tc1, self.eos, "Snargate")
    ...
    while True: # run forever......
        ...
        if dsc_queue: # there's something to send....
            ...
            if tx_ok == 1: # we can transmit.....
                ...
                ######################
                #try:
                ### 6/5/16 --- send a RTTY carrier for 3 seconds - should trigger an ATU "PTT Tune" and also
                ### allows us to read the resulting PWR and SWR meter readings
                set_mode("rtty")
                ptt_on()
                time.sleep(5)
                sql_time = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(time.time()))
                pwr=get_pwrm()
                swr=get_swr()
                print "PWR : %s, SWR : %s" % (pwr, swr)
                sql.load_txpwr(sql_time, tx_freq, pwr, swr)
                ptt_off()
                ### change to USB mode / digi_on -just in case it isn't already
                set_mode("usb")
                set_digi()
                ...
                ####
                ### 5/6 16 - send a 1 second tune carrier - but this might be unnecessary
                ### now that the ATU is in "PTT Tune mode".... tests needed to confirm
                ### 
                ### perhaps the final solution will be:
                ###
                ### set_mode("usb")
                ### set_digi()
                ### ptt_on()
                ### tune_carrier(0.7,5) # how to read the swr meter though?
...
class ThreadedRequestHandler(SocketServer.StreamRequestHandler):
    ...
    def admin(self, command):
        ...
        elif self.command[1] == "UPDATEMOTD":
            self.motd_add = self.command[2]
            self.respond="Added new update to MOTD : %s" % self.motd_add
            self.send_response(self.respond)
            update_motd(self.motd_add+"\n")
		...
		######## A Combined STATUS poll 
            ...
            last_ck = sql.get_last_ck()
            #print last_ck
            # change RX-ID into "via RX_ID"
            rx_id = last_ck[-1][-1]
            #print rx_id
            # test that there's something in the list
            if last_ck != "":
                # send back the final element of the list to the client
                try:
                    self.last_ck="%s" % " ; ".join([str(x) for x in last_ck[-1][0:-1]] )
                    self.last_ck_reply = self.last_ck+" (via "+rx_id+")"
                except:
                    self.last_ck_reply="Nothing Yet"
            # otherwise there has been nothing received
            else:
                # inform the client that there's nothing to show
                self.last_ck_reply="Nothing Received"
            
            
            self.message_count_reply= "Sent : %d,  Received : total  %d, to us %d / from us %d (Count reset : %s)" % (sum(message_per_band_list), messages_received_list[0], messages_received_list[1], messages_received_list[0] - messages_received_list[1], messages_received_list[2])
            ...
            self.respond=self.online_reply+"~"+self.tx_state_reply+"~"+self.get_ptt_reply+"~"+self.last_rx_reply+"~"+self.last_tx_reply+"~"+self.message_count_reply+"~"+self.perband_reply+"~"+self.freq_list_reply+"~"+self.last_ck_reply
        ...
        # a command to retrieve entries from the "TXPWR" log - records TX Power and SWR at the beginning of each transmission
        # this is stored in a SQLite database
        # the function sql.get_tx() is set to retrieve a limited list (based on number)
        # from the database.   
        elif self.command[1] == "GETALLTXPWRSQL":
            self.sql = sql.dump_txpwr()   
            self.result = ""
            for row in self.sql:
                # each record has "datatime, frequency : pwr : swr
                # append each line to the response string
                self.result += row[0]+" ; "+row[1]+" ; "+row[2]+" ; "+row[3]+"\n"
    ...
    def reply(self, rx_freq, fmt, to_mmsi, cat, from_mmsi, tc1, tc2, data, eos):
        ...
        if new_dsc_call in dsc_queue:
            ...
            if tx_ok == 1:
                ...
                sql.load_rxtx(self.sql_time, rx_freq, "TX", to_mmsi, self.name, self.call, tc1, eos, "Snargate")
...
if __name__ == "__main__":
    ...
    ### only allow UPD packets from the tap0 VPN interface
    HOST, PORT = "192.168.21.151", 50669
    
    # the TCP Server host/port assignment 
    ### allow access from the any interface as well as the tap0 VPN interface
    ### to allow someone on the local LAN to use the GUI admin facility
    ###
    address = ('', 50669)
