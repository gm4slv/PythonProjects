...
def get_last_ck():
    ...
    with db:
        ...
        cur.execute("SELECT datetime, freq, mmsi, name, call, tc1, eos, id from checklog ORDER BY 1 DESC LIMIT 1")
        results = cur.fetchall()
...
def load_txpwr(time, freq, pwr, swr):
    db = sqlite3.connect("txpwr.db", timeout=5)
    cur = db.cursor()
    cur.execute("INSERT INTO txpwr (datetime, freq, pwr, swr) VALUES (?,?,?,?)", (time, freq, pwr, swr))
    db.commit()
    db.close()  
    
def get_txpwr():
    db = sqlite3.connect("txpwr.db")
    with db:
        cur = db.cursor()
        
        cur.execute("SELECT * FROM (SELECT * FROM txpwr  ORDER BY 1 DESC LIMIT 50) sub ORDER BY 1 ASC")
        results = cur.fetchall()
...
def dump_txpwr():
    ...
    with db:
        ...
        cur.execute("SELECT * FROM txpwr ORDER BY 1 ASC")
        results = cur.fetchall()
