    ...
    def handle(self):
        ...
        while True:
            ...
            # identifying which radio object to apply the method to
                ...
                my_radio = eval(words[-1])
                nblank = my_radio.get_nb()
                self.wfile.write(nblank)
            elif words[0] == "getnr":
                my_radio = eval(words[-1])
                nrstate = my_radio.get_nr()
                self.wfile.write(nrstate)
            elif words[0] == "getnrl":
                my_radio = eval(words[-1])
                nrlevel = my_radio.get_nr_l()
                self.wfile.write(nrlevel)
                
            elif words[0] == "setnrl":
                my_radio = eval(words[-1])
                snrl = words[1]
                nrl = my_radio.set_nr_l(snrl)
                self.wfile.write(nrl)
                
            elif words[0] == "getanf":
                my_radio = eval(words[-1])
                anfstate = my_radio.get_anf()
                self.wfile.write(anfstate)
            elif words[0] == "nbon":
                my_radio = eval(words[-1])
                nb = my_radio.nb_on()
                self.wfile.write(nb)
                ...
                my_radio = eval(words[-1])
                anf = my_radio.anf_on()
                self.wfile.write(anf)
            elif words[0] == "anfoff":
                my_radio = eval(words[-1])
                anf = my_radio.anf_off()
                self.wfile.write(anf)
            ...
            elif words[0] == "getpwrmtr":
                my_radio = eval(words[-1])
                pwrmtr = my_radio.get_pwrmtr()
                self.wfile.write(pwrmtr)
                ...
                my_radio = eval(words[-1])
                sbw = words[1]
                bw = my_radio.set_bw(sbw)
                self.wfile.write(bw)
            
            elif words[0] == "ptton":
                my_radio = eval(words[-1])
                pon = my_radio.ptt_on()
                self.wfile.write(pon)

            elif words[0] == "pttoff":
                my_radio = eval(words[-1])
                poff = my_radio.ptt_off()
                self.wfile.write(poff)

           
            elif words[0] == "digon":
                my_radio = eval(words[-1])
                don = my_radio.dig_on()
                self.wfile.write(don)
            
            elif words[0] == "digoff":
                my_radio = eval(words[-1])
                doff = my_radio.dig_off()
                self.wfile.write(doff)
            
            elif words[0] == "getdig":
                my_radio = eval(words[-1])
                digstate = my_radio.get_dig()
                self.wfile.write(digstate)
