...
class Icom(object):
    ...
    def dig_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + digi_on_cmd + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def get_dig(self):
        sendStr = preamble + preamble + self.radio_address + controller + get_dig_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        if result[6] == "\x00":
    ...
    def get_nb(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nb_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        if result[6] == "\x00":
    ...
    def get_nr(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nr_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        if result[6] == "\x00":
    ...
    def get_anf(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_anf_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        if result[6] == "\x00":
    ...
    def get_pwrmtr(self):
        sendStr = preamble + preamble + self.radio_address + controller + get_pwrmtr_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        p1 = ord(result[7]) / 16
    ...
    def get_nr_l(self):
        sendStr = preamble + preamble + self.radio_address + controller + nr_level_cmd + eom
        result = self.tx_rx(sendStr)
        if not result:
            return "0"
        p1 = ord(result[7]) / 16
    ...
    def set_nr_l(self, nrl):
        ...
        sendStr = preamble + preamble + self.radio_address + controller + nr_level_cmd + snrl + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return self.get_nr_l()
        elif result[4] == nak:
            return "NAK received"
    ...
    def get_bw(self):
        sendStr = preamble + preamble + self.radio_address + controller + bw_cmd + eom
        result = self.tx_rx(sendStr)
        #print result
        #p1 = ord(result[7]) / 16
        #p2 = ord(result[7]) % 16
    ...
    def set_bw(self, bw):
        ...
        sbw4 =  (int(bw) % 10)
        sbw = chr(sbw3+sbw4)
        sendStr = preamble + preamble + self.radio_address + controller + bw_cmd + sbw + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return self.get_bw()
        elif result[4] == nak:
    ...
    def nb_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nb_cmd + set_nb_on + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def nb_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nb_cmd + set_nb_off + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def nr_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nr_cmd + set_nr_on + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def nr_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_nr_cmd + set_nr_off + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def anf_off(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_anf_cmd + set_anf_off + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
    ...
    def anf_on(self):
        sendStr = preamble + preamble + self.radio_address + controller + set_anf_cmd + set_anf_on + eom
        result = self.tx_rx(sendStr)
        if result[4] == ack:
            return "Success"
        elif result[4] == nak:
