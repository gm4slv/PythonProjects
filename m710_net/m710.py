...
class m710(object):
    ...
    def remote_on(self):
        ...
        #print "in remote_on() - result ", result
        if result:
            return "Remote On"
        else:
            #print "in remote_on() - calling myself due to error"
    ...
    def get_rxfreq(self):
        ...
        ecc = self.get_ecc(command)
        sendStr = preamble+controller+","+radio+","+command+"*"+ecc+cr+lf
        #print "in get_rxfreq(), sendStr ", sendStr
        result = self.tx_rx(sendStr)
        #print "in get_rxfreq() , result = ", result
        if result:
            list = result.split(",")
            f = list[4].split("*")[0]
    ...
    def set_rfg(self,v):
        ...
        if result:
            ...
            return self.set_rfg(v)

    def tune(self):
        self.cur_mode = self.get_mode()
        self.set_mode("FSK")
        self.cur_pwr = self.get_txpower()
        self.tune_time = 3
        self.set_txpower("1")
        self.ptt_on()
        time.sleep(self.tune_time)
        self.ptt_off()
        self.set_mode(self.cur_mode)
        self.set_txpower(self.cur_pwr)
        return "Tune Cycle Complete"
    ...
    def tx_rx(self, sendStr):
        ...
        lock.release()
        #print "tx_rx() result ", result
        reply = result.split("*")    
        #print "tx_rx() reply ", reply
        if len(result) > 1:
            ecc = reply[1][0:2].lower()
            message = reply[0]            
            if self.check_ecc(message, ecc):
                #print "tx_rx() returning ", result
                return result

        else:
            #print "tx_rx() returning False"
            return False
