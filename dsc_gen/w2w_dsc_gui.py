...
# Generic, non-TX GUI, without CW ID
...
class Application(Frame):
    def __init__(self, master):
        ...
        self.txqueue.put(0)
        m1 = threading.Thread(target = self.send_dsc)
        m1.setDaemon(True)
        m1.start()
    ...
    def create_widgets(self):
        ...
        self.go_b = Button(self, text = "Send", command = self.txqueue_on)
        ...
        self.cw_call_e.grid(row = 10, column = 2, columnspan = 2)
        self.cw_call_e.insert(0, "qtc")
        
        self.cw_b = Button(self, text = "CW", command = self.send_cwid)
...
if __name__ == '__main__':
    ...
    root.title("Wire2Waves MF/HF DSC " + version)
    root.resizable(0, 0)
